...
class YuNetHead_relu(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, obj_conv, stride_this_level, x) in enumerate(
            ...
            cls_output = cls_conv(cls_x)

            reg_output = reg_conv(reg_x)

            obj_output = obj_conv(obj_x)
    ...
    def get_geometry_constraint(
        ...
        # Dynamic adjustment of center_radius
        center_radius = 1.5
        ...
        while True:
            ...
            # Check if we have any positive anchor filters
            if anchor_filter.any():
                break
            else:
                # Increase the radius if no anchors are matched
                center_radius += 0.25
                center_dist = expanded_strides_per_image.unsqueeze(0) * center_radius
                print("Increasing center radius to:", center_radius)
